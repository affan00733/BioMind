...
def evaluate_evidence_support(hypothesis, evidence_texts):
    ...
    biomedical_indicators = [
        'protein', 'gene', 'drug', 'disease', 'mechanism',
        'pathway', 'target', 'inhibition', 'activation'
    ]
    quality_score = 0
...
def evaluate_confidence(hypothesis, evidence_texts):
    ...
    weights = {"evidence": 0.4, "consistency": 0.35, "novelty": 0.25}
    weighted_score = (evidence_score * weights["evidence"]) + (
    ...
    logging.info(
        ...
        evidence=f"{evidence_score:.3f}",
        consistency=f"{consistency_score:.3f}",
        novelty=f"{novelty_score:.3f}",
        weights=weights,
        final=f"{confidence_percentage:.2f}%",
    )

    # Return a structured result so callers can show the breakdown in the UI
    return {
